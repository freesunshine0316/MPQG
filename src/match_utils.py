...
def collect_final_step_lstm(lstm_rep, mask):
    col_idx = tf.cast(tf.reduce_sum(mask, axis=-1)-1, tf.int32) #[batch,]
    col_idx = tf.maximum(col_idx, tf.zeros_like(col_idx, dtype=tf.int32)) # [batch,]
    row_idx = tf.range(0, limit=tf.shape(mask)[0]) # [batch,]
    indices = tf.stack((row_idx,col_idx,), axis=1) # [batch_size, 2]
...
def match_passage_with_question(passage_context_representation_fw, passage_context_representation_bw, mask,
    ...
    if with_full_matching:
        ...
        if with_forward_match:
            fw_question_full_rep = collect_final_step_lstm(question_context_representation_fw, question_mask)
